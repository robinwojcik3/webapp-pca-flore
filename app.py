...
# --- Sélection des Habitats (au-dessus du data_editor) ---
current_releves_df_for_checkboxes = st.session_state.releves_df.copy() # Utiliser une copie pour la logique des checkboxes
selected_col_indices_for_analysis = st.session_state.selected_habitats_indices # Init avec l'état de session
...
if not current_releves_df_for_checkboxes.empty and \
   ...
   len(current_releves_df_for_checkboxes) > 0:
    ...
    st.markdown("**Sélectionnez les habitats (colonnes du tableau ci-dessous) à inclure dans l'analyse :**")
    
    cols_per_row_for_checkboxes = min(7, num_actual_cols if num_actual_cols > 0 else 1)
    if num_actual_cols > 0:
        ...
        if sorted(temp_selected_indices_from_checkboxes) != sorted(st.session_state.selected_habitats_indices):
            ...
            st.rerun() # Rerun pour mettre à jour l'état du bouton d'analyse, etc.
    else:
...
# S'assurer que releves_df a des noms de colonnes de type str pour data_editor
...
if not edited_releves_df_from_editor.equals(st.session_state.releves_df):
    ...
    # Si le nombre de colonnes change, réinitialiser les sélections
...
with col_controls_area:
    ...
    if not selected_col_indices_for_analysis and not ref.empty:
        st.info("Veuillez sélectionner au moins un habitat (colonne) ci-dessus pour activer l'analyse.")

fig_pca = None
fig_dend = None
species_binom_user_unique = [] # Recalculé à chaque analyse
...
if run_main_analysis_button and not ref.empty and selected_col_indices_for_analysis:
    ...
    if not ref_binom_series.empty:
        # S'assurer que ref_binom_series est bien une Series avec un index simple pour reset_index
    ...
    if st.session_state.sub.empty:
        with col_controls_area: st.error("Aucune des espèces sélectionnées n'a été trouvée dans la base de traits de référence. L'analyse ne peut continuer.")
        st.session_state.run_main_analysis_once = False; st.stop()
    if st.session_state.sub.shape[0] < n_clusters_selected and n_clusters_selected > 0 :
        with col_controls_area: st.error(f"Le nombre d'espèces uniques trouvées et utilisables ({st.session_state.sub.shape[0]}) est inférieur au nombre de clusters demandé ({n_clusters_selected}).");
        st.session_state.run_main_analysis_once = False; st.stop()
    if st.session_state.sub.shape[0] < 2:
        with col_controls_area: st.error(f"Au moins 2 espèces uniques sont nécessaires pour l'analyse. {st.session_state.sub.shape[0]} espèce(s) trouvée(s) et utilisée(s).");
        st.session_state.run_main_analysis_once = False; st.stop()
    ...
    try:
        ...
        if sub_for_analysis.empty or sub_for_analysis.shape[1] == 0:
            with col_controls_area: st.error(f"Aucun trait numérique trouvé pour les espèces sélectionnées dans la base de référence. L'analyse ACP ne peut pas être effectuée.")
            st.session_state.run_main_analysis_once = False; st.stop()
        ...
        if not isinstance(coords_df, pd.DataFrame):
            if isinstance(coords_df, np.ndarray) and coords_df.ndim == 2 and coords_df.shape[0] == len(st.session_state.sub):
                ...
                with col_controls_area: st.error("Les coordonnées PCA retournées par core.analyse ne sont pas dans un format attendu ou de longueur incorrecte.")
                st.session_state.run_main_analysis_once = False; st.stop()
        ...
        else: # Fallback si 'Espece' n'est pas dans sub
        ...
        if not ecology_df.empty:
            ...
            if ecology_df.index.name == 'Espece_norm' and 'Description_Ecologie' in ecology_df.columns:
                ...
                if ecology_df.index.name != 'Espece_norm':
                     st.warning("L'index du DataFrame écologique n'est pas 'Espece_norm'.")
                if 'Description_Ecologie' not in ecology_df.columns:
                     st.warning("La colonne 'Description_Ecologie' est manquante dans le DataFrame écologique.")
            
            current_pdf['Ecologie'] = current_pdf['Ecologie_raw'].apply(lambda x: format_ecology_for_hover(x))
            current_pdf['Ecologie'] = current_pdf['Ecologie'].fillna(format_ecology_for_hover(None)) # Utiliser None pour le fillna
        else:
            current_pdf['Ecologie'] = format_ecology_for_hover(None) # Utiliser None
        ...
        if hasattr(pca_results, 'components_') and hasattr(pca_results, 'explained_variance_') and \
           ...
           pca_results.components_.size > 0 and pca_results.explained_variance_.size > 0 :
            ...
            # S'assurer que components_ est 2D
            ...
            if loadings.ndim == 1 : loadings = loadings.reshape(-1,1) # S'assurer que loadings est 2D pour sum(axis=1)
            ...
            st.session_state.vip_data_df_for_calc = pd.DataFrame(columns=["Variable", "Communalité (%)"])
            st.warning("Les résultats PCA ne contiennent pas les informations nécessaires pour calculer les communalités (components_ ou explained_variance_ manquants/vides).")
        ...
        with col_controls_area: st.error(f"Une erreur est survenue lors de l'analyse principale : {e}"); st.exception(e)
        st.session_state.run_main_analysis_once = False; st.stop()
...
if st.session_state.run_main_analysis_once:
    ...
    if isinstance(X_for_dendro_display, np.ndarray) and X_for_dendro_display.ndim == 2 and X_for_dendro_display.shape[0] > 1 and X_for_dendro_display.shape[1] > 0:
        try:
            ...
            fig_dend = ff.create_dendrogram(X_for_dendro_display, orientation="left", labels=dendro_labels, linkagefun=lambda _: Z, color_threshold=dyn_thresh if n_clusters_selected > 1 else 0, colorscale=COLOR_SEQUENCE)
        ...
        except Exception as e: print(f"Erreur lors de la création du dendrogramme: {e}"); fig_dend = None
    else: fig_dend = None
...
if st.session_state.run_main_analysis_once and not st.session_state.get('sub', pd.DataFrame()).empty:
    st.markdown("---"); st.subheader("Étape 3: Exploration Interactive des Variables")
    ...
    with col_interactive_table:
        ...
        if not df_editor_source.empty:
            # S'assurer que snapshot est un DataFrame avec les bonnes colonnes
        ...
        if not numeric_traits_plot: st.warning("Aucun trait numérique trouvé pour l'exploration interactive.")
            ...
            if not all(col in pdf_plot.columns for col in required_pdf_cols): st.warning("Colonnes requises (Espece_User, Ecologie, Cluster) manquent dans les données PCA.")
                ...
                if duplicates_mask.any():
                    # ... (jitter logic as before, ensure float conversion) ...
